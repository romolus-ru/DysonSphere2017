// TODO debugView сделать как toolbar - перемещаемая кнопка, которая разворачивается или в окно или набор кнопок, и по каждой кнопке что-нибудь показывается
...
// TODO у окна надо сделать элемент для изменения размера и что бы это можно было отключить/настраивать (скорее всего по умолчанию отключить, а включать изменением поля)
// TODO компонент dragable при запуске перемещения должен отключить все другие события - а то сейчас кнопки срабатывают
