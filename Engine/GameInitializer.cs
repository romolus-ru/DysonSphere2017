//	окно исследований доделать - сортировка айтемов по купленности 
// и верхнее меню - расставить кнопки в нужном порядке
//добавить обработку исследования увеличения количества дополнительных заказов
// проверить формирование заказов - разброс большой, 

// сделать научные предметы - вначале они "покупаются" и влияют на все корабли и не только. 
// потом когда куплены нужные айтемы уже появляется магазин и возможность покупать улучшения
// научные айтемы покупаются за награды от завершенных заказов. магазин должен быть доступен не сразу
// таким же образом увеличивается количество доступных кораблей
// +кличество кораблей +магазин +ассортимент магазина (доступ к апгрейдам более высокого уровня) +уровень заказов

// FilteredScrollViewWindow ViewScroll сделать приватным
// для автоматических кораблей ввести признак - что если корабль прилетел по требованию игрока - он будет стоять в доке ока его не запустят
...
// сделать картинку верхнего меню, где будут располагаться знаки, опыт, и т.п.
// окно просмотра характеристик имеющихся ресурсов (возможно, они будут уменьшены до 5 максимум в каждой категории)
// обновление заказа возможно стоит сделать когда заказ выполнен и когда ниодин из кораблей не связан с этим заказом

// окно просмотра "товаров" и их количества с возможностью купить/обменять (магазин)
// окно редактирования корабля с просмотром текущих улучшений, всех имеющихся улучшений и перетаскиванием улучшений в нужные слоты

// за выполненный заказ начисляется (знак выполненного контракта). потом будут выдаваться особые знаки выполненного контракта
// начиная с определенного уровня игрока становится доступным магазин, в котором можно купить модули для космолёта
// до открытия  магазина эти знаки можно обменять на награду - такие же модули, но случайные.
// на самом деле первые модули будут не случайными - что бы игрок мог оснастить корабли лучшим образом на ранних стадиях игры
// вначале доступен 1 корабль и 4 сломанные - их можно починить за знаки выполненного контракта
// потом до открытия магазина можно будет купить ещё 5 кораблей (появится кнопка покупки корабля). всего можно будет получить 20 кораблей
...
// - высшего класса - увеличивает почти все характеристики, а 1 очень сильно (заменяет 3-5 обычных модулей). улучшенный это урезанный высший модуль
...
// перенести игру в новый проект SpaceConstruction - и подключить новые классы
//выбирать заказ случайно и заполнять его - количество товара и планеты. сейчас  всё находится в неймспесе ResourceNew
//так же сам заказ сохранять и у планеты и у заказов что бы централизованно контролировать
...
// у описания заказа будет только указываться группа материалов и количество будет формироваться уже исходя из текущих условий и из доступных материалов в группе
