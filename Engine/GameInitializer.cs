// сделать общую карту с переходами. у переходов есть условия их открытия
// объединяются линиями, но у которых будут условия показа на экран и условия активации, и подсказки разные
// сделать редактор карты. с расстановкой точек, геометрий и т.п.
// ракета запускается и поражает статическую цель
// сохраняется результат прохождения карты - дата прохождения, время прохождения, состояние статичных точек
// GetSubmarineMapCollision - сделать определение что корабль отдаляется от преграды и разрешить движение
...
// переделать по возможности ContainsKey в TryGetValue для словарей
