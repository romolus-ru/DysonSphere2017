// ViewText - заполняется и выводит текст.
...
// класс для вывода туториальных ачивок (как системные объекты должны выводиться
// класс для вывода информации посередине экрана крупно
// класс для вывода подсказок

// сделать дополнительный класс для вывода текста. внутри он будет содержать куски текста, 
// каждый из которых будет что то уметь делать (выводить текст, картинку, переходить на новую линию и т.п)
// желательно что бы можно ещё было группировать и менять смещение (т.е. вложенные группы)
