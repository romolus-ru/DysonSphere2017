// для Init у объектов сделать log если вызывается инициализация второй раз. и она должна блокироваться - т.е. вызываться только 1 раз. 
// сейчас в ViewManager создаётся объект и инициализируется сначала в AddComponent и в Init
