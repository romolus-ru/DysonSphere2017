// расширить идентификатор у collectorclass до int - он сейчас уменьшается до ushort
// не сканируется orders из exe файла
// фильтр сделать у окна редактирования коллектора (он сейчас не работает
// сделать кнопку выбора файла - может быть ситуация что класс переместился а имя и т.п. осталось
...
// сделать окно для выбора EventBase из коллектора с возможностью просканировать файлы и сохранить информацию в базу
// сделать атрибут у айтема который будет применяться для строк и указывать модификатор элемента редактирования, что бы был не редактор строки а вывод строки с кнопкой выбора имени класса из коллектора (в данном случае в eventBase)

// ToolsCollectorManager - там сканировать файл и там добавлять если класса нету
// collector должен уметь логировать свои действия
// при сохранении секции надо убеждаться что данный тип данных есть в системе. 
// искать этот класс и сохранить. если не получится то не сохранять и выводить сообщение об этом
// в stateEngine добавить вывод подсказок - что бы все могли туда обратиться и вывести подсказку
