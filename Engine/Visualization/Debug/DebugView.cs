...
using Engine.Visualization.Debug.Modules;

namespace Engine.Visualization.Debug
{
	public class DebugView:ViewWindow
	{
		...
		private void ViewButtons()
		{
			...
			if (_buttons==null) {
				...
				string[] names = { "ComponentsView", "EventsView", "KeysView", "StateClientView" };
				ViewButton btn;
		...
		private void SwitchWindow(string name)
		{
			...
			if (name == "StateClientView") {
				if (_windows == null || !_windows.ContainsKey(name)) {
					window = new StateClientView();
				}
			}
